...
func confirm() -> void:
	main_scene.add_data(date_field.text, time_field.text)
	
	time_field.queue_free()
	date_field.queue_free()
	window.queue_free()
